...
class VoiceClient(discord.VoiceClient):
    def __init__(self, client: ChickBot, channel: discord.abc.Connectable):
        self.client = client
...
class Music(commands.Cog):
    ...
    async def cog_before_invoke(self, ctx: commands.Context[ChickBot]) -> None:
    ...
    async def cog_command_error(self, ctx: commands.Context[ChickBot], error):
        if isinstance(error, commands.CommandInvokeError):
            await ctx.send(error.original)

    async def ensure_voice(self, ctx: commands.Context[ChickBot]) -> None:
    ...
    async def track_hook(self, event: lavalink.events.BaseEvent):
        if isinstance(event, lavalink.events.TrackStartEvent):
            ...
            if c:
                ...
                embed = (
                    discord.Embed(
                        colour=self.bot.color, title="Now Playing", color=self.bot.color
                    )
                    .add_field(
                        name="Song", value=f"[{event.track.title}]({event.track.uri})"
                    )
                    .add_field(
                        name="Duration",
                        value=str(lavalink.utils.format_time(event.track.duration)),
                    )
                    .set_footer(
                        text=f"Requested by {requester.name}",
                        icon_url=requester.display_avatar.url,
                    )
                    .set_thumbnail(url=f"{await thumbnail(event.track.identifier)}")
                )
    ...
    async def join(self, ctx: commands.Context[ChickBot]):
    ...
    @commands.hybrid_command()
    async def pause(self, ctx: commands.Context[ChickBot]) -> None:
        """Pauses the player."""
        player = self.bot.lavalink.player_manager.get(ctx.guild.id)
        if not player.is_playing:
    ...
    @commands.hybrid_command()
    async def resume(self, ctx: commands.Context[ChickBot]) -> None:
        """Resumes the player."""
        player = self.bot.lavalink.player_manager.get(ctx.guild.id)
        if not player.is_playing:
            return await ctx.send("Not playing.", delete_after=10)
        ...
        await ctx.send(
            "\N{BLACK RIGHT-POINTING TRIANGLE WITH DOUBLE VERTICAL BAR} | Resumed.",
            delete_after=10,
        )

    @commands.hybrid_command()
    async def skip(self, ctx: commands.Context[ChickBot]) -> None:
        ...
        await player.skip()
        await ctx.send(
            "\N{BLACK RIGHT-POINTING DOUBLE TRIANGLE WITH VERTICAL BAR} | Skipped.",
            delete_after=10,
        )

    @commands.hybrid_command()
    async def stop(self, ctx: commands.Context[ChickBot]) -> None:
    ...
    @commands.hybrid_command()
    async def volume(self, ctx: commands.Context[ChickBot], volume: int = None):
        """Changes the player's volume."""
        player = self.bot.lavalink.player_manager.get(ctx.guild.id)
        if not volume:
            return await ctx.send(f"\N{SPEAKER} | {player.volume}%", delete_after=10)
        await player.set_volume(volume)
        await ctx.send(f"\N{SPEAKER} | Set to {player.volume}%", delete_after=10)

    @commands.hybrid_command()
    async def shuffle(self, ctx: commands.Context[ChickBot]) -> None:
    ...
    @commands.hybrid_command()
    async def queue(self, ctx: commands.Context[ChickBot], page: int = 1):
        player = self.bot.lavalink.player_manager.get(ctx.guild.id)
        if not player.queue:
        ...
        embed.set_footer(
            text=f"Viewing page {page}/{pages}, do {ctx.prefix}queue <page> to view a different page."
        )
