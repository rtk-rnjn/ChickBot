...
from core import ChickBot, Cog, ConfirmView

from .utils import EmojiURL, emoji_name
...
class Utility(Cog):
    ...
    async def lock(self, ctx: commands.Context[ChickBot], channel: discord.TextChannel):
        """
    ...
    @commands.has_permissions(manage_channels=True)
    async def unlock(
        self, ctx: commands.Context[ChickBot], channel: discord.TextChannel
    ):
        """
        Unlocks a channel
    ...
    @commands.has_permissions(manage_channels=True)
    async def nuke(self, ctx: commands.Context[ChickBot]):
        """
        Nukes the channel
        """
