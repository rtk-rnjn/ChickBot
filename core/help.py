...
class HelpCommand(commands.HelpCommand):
    ...
    async def send_bot_help(
        self, mapping: Mapping[Cog, List[commands.Command]]
    ) -> None:
        ctx = self.context
        hidden = "Developer"
        ...
        embed.description = (
            f"Use `{ctx.prefix}help <command>` for more information about a command."
        )
        ...
        embed.add_field(
            name=f"{ctx.bot.user.name} Slash Commands",
            value=", ".join(slash_cmds),
            inline=False,
        )
